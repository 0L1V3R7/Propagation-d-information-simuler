...
with st.spinner("CHARGEMENT DE LA PAGE"):
    ...
    colone = st.bottom.slider("Ajuster les affichage",4,6,5)
    ...
    colon1,colon2 = st.columns([10-colone,colone])
        ...
        with colon1 : 
            if st.session_state.step != [[]]  and st.session_state.pos is not None:
                if cinem == "Manuel":
                    ...
                    with colon2:
                        if st.session_state.courbe is not None :
                            fig = tracer_evolution_temporelle_cinem(cs,step)
                            st.pyplot(fig)
                            plt.close(fig)
        ...
        with colon1:
            if st.session_state.stepbis != [[]]  and st.session_state.pos is not None:
                if cinem == "Manuel":
                    ...
                    with colon2:
                        if st.session_state.courbebis is not None :
                            fig = tracer_evolution_temporelle_cinembis(cbs,step)
                            st.pyplot(fig)  
                            plt.close(fig)      
